...
const LM_CALLQ=[['connected','Pickups','any answer'],['conversations','Conversation','≥75s'],['deepConv','Deep','≥3 min'],['qualifying','Qualifying','≥10 min']];
const LM_CALLKEYS={connected:1,conversations:1,deepConv:1,qualifying:1};
const LM_XTILES={connected:{k:'connected',label:'Pickups (any answer, incl. voicemail)',fmt:'num'},deepConv:{k:'deepConv',label:'Deep ≥3 min',fmt:'num'}};
const lmTileDef=k=>LM_TILES.find(t=>t.k===k)||LM_XTILES[k]||{k:k,label:k,fmt:'num'};
function lmSpeedBySource(lm,t){var bs=lm.bySource;if(!bs||!bs.length)return '';return '<div class="lm-qh">By lead type · fastest first · inbound ≠ cold-list</div><div class="lm-ladder">'+bs.map(function(b){return '<span class="lm-lad"><span>'+b.source+' ('+b.n+')</span><b>'+LM_FMT[t.fmt](b.avg)+'</b></span>';}).join('<i>·</i>')+'</div>';}
...
/* rail formatter: counts show 1 decimal when small/fractional (so a scaled daily target like 0.5 doesn't round to 0) */
function lmRailN(t,v){if(v==null)return null;if(t.fmt==='num')return (v>0&&v<10)?(Math.round(v*10)/10).toLocaleString():lmNum(v);return LM_FMT[t.fmt](v);}
function lmTileBars(lm,t){var g=lm.game,a=(lm.bar==null)?null:lmRailN(t,lm.bar),tg=(g&&g.bars&&g.bars.target!=null)?lmRailN(t,g.bars.target):null;if(a==null&&tg==null)return '';var p=[];if(a!=null)p.push('avg <b>'+a+'</b>');if(tg!=null)p.push('🎯 <b>'+tg+'</b>');return '<div class="lm-t-bars">'+p.join('<i>·</i>')+'</div>';}
...
function lmReachCard(M,lens){const L=M.lenses[lens];const tc=L.trueConnects,tr=L.trueConnectRate,rc=L.speed&&L.speed.reach;if((!tc||tc.current==null)&&!rc)return '';let h='<div class="sec-h"><h2>Reach</h2><span class="tag">diagnostic · not a competition — rate depends on lead mix</span></div><div class="lm-reach card">';h+='<div class="lm-reach-row"><span>True connects (two-speaker verified)</span><b>'+(tc&&tc.current!=null?lmNum(tc.current):'—')+'</b></div>';h+='<div class="lm-reach-row"><span>Connect rate (true connects ÷ dials)</span><b>'+(tr&&tr.current!=null?(tr.current*100).toFixed(1)+'%':'—')+'</b></div>';if(rc){h+='<div class="lm-reach-row"><span>Leads reached (attempt→connect)</span><b>'+rc.connected+' / '+rc.attempted+' ('+Math.round(rc.rate*100)+'%)</b></div>';if(rc.inbound.att)h+='<div class="lm-reach-row sub"><span>· inbound leads</span><b>'+rc.inbound.con+' / '+rc.inbound.att+'</b></div>';if(rc.cold.att)h+='<div class="lm-reach-row sub"><span>· cold list (MLS)</span><b>'+rc.cold.con+' / '+rc.cold.att+'</b></div>';if(rc.never.length)h+='<div class="lm-reach-never"><span>Not yet reached ('+rc.never.length+') — coaching targets:</span> '+rc.never.slice(0,12).join(', ')+(rc.never.length>12?' +'+(rc.never.length-12)+' more':'')+'</div>';}h+='</div>';return h;}
function lmHome(M,person,lens,team){return '<div class="lm-head"><div><p class="eyebrow">LM Scoreboard</p><h1>'+person+'</h1></div>'+lmPicker(person,team)+'</div><div class="lm-clockbar">this device&rsquo;s clock → <span id="lm-clock">'+lmNowLbl()+'</span></div>'+lmLensBar(person,lens)+lmComp(M,person,lens)+'<section class="lm-hero">'+lmStreakHL(M,lens)+lmPaceDay(M)+'</section><div class="sec-h"><h2>Scoreboard</h2><span class="tag">'+LM_LBL[lens]+' · tap a tile</span></div><section class="lm-tiles">'+LM_TILES.map(t=>lmTile(M,person,lens,t)).join('')+'</section>'+lmReachCard(M,lens)+'<button class="lm-deals-btn focusbtn" onclick="lmGo(\''+person+'\',\''+lens+'\',\'focus\')">💰 Make More Money — your earning levers →</button><button class="lm-deals-btn" onclick="lmGo(\''+person+'\',\''+lens+'\',\'deals\')">⊞ Deals &amp; Pipeline — '+lmMoney(M.comp.totalOpportunity)+' in play →</button><div class="lm-lb">Solo · self-competition. Head-to-head ranking activates when a 2nd LM joins.</div>';}
function lmDetail(M,person,lens,key){const t=lmTileDef(key),lm=M.lenses[lens][key],g=lm.game;let h='<button class="lm-back" onclick="lmGo(\''+person+'\',\''+lens+'\',\'\')">‹ Scoreboard</button><div class="lm-head"><div><p class="eyebrow">'+(lens==='today'?lmRel(lm.anchorDate):LM_LBL[lens])+'</p><h1>'+t.label+'</h1></div>'+lmLensBar(person,lens,key)+'</div><div class="lm-big '+lmPaceCls(lm.paceState)+'">'+LM_FMT[t.fmt](lm.current)+'</div><div class="lm-cmp">'+lmDelta(lm.comparePct,t.low)+' vs '+LM_PRIOR[lens]+' ('+(lm.comparisonEst?'~':'')+LM_FMT[t.fmt](lm.comparison)+(lm.comparisonEst?' pace':'')+')</div>'+lmBars(lm,t)+(LM_CALLKEYS[key]?lmCallLadder(M,lens,key):'')+(key==='speed'?lmSpeedBySource(lm,t):'');if(g){h+='<div class="lm-gcards">';if(g.streak)h+='<div class="lm-gc"><div class="lm-gl">Streak</div><div class="lm-gv">'+g.streak.current+'</div><div class="lm-gs">best '+g.streak.best+' '+LM_UNIT[lens]+'s</div></div>';if(g.pr&&g.pr.best)h+='<div class="lm-gc"><div class="lm-gl">Personal record</div><div class="lm-gv">'+LM_FMT[t.fmt](g.pr.best.value)+'</div><div class="lm-gs">'+g.pr.best.label+(g.pr.toTie?' · '+LM_FMT[t.fmt](g.pr.toTie)+' to tie':(g.pr.isRecord?' · NEW 🎉':''))+'</div></div>';if(g.paceToBeat&&g.paceToBeat.remaining!=null&&g.paceToBeat.daysLeft>0)h+='<div class="lm-gc"><div class="lm-gl">Pace to beat '+g.paceToBeat.basis+'</div><div class="lm-gv">'+(Math.round(g.paceToBeat.perDayNeeded*10)/10)+'/day</div><div class="lm-gs">'+LM_FMT[t.fmt](g.paceToBeat.remaining)+' over '+g.paceToBeat.daysLeft+'d</div></div>';h+='</div>';}h+='<div class="card chart-card"><h3>'+t.label+' · '+LM_LBL[lens]+'</h3><p class="sub">'+(RATIO2[key]?'two lines = the counts behind the % (e.g. showed vs booked)':'solid = you · dashed = your avg'+(g&&g.bars&&g.bars.target!=null?' · dotted = target':''))+'</p><div class="chart-wrap" onclick="lmFsOpen()"><canvas id="lmchart"></canvas><span class="chart-expand">⤢ tap to expand</span></div></div><div class="lm-diag"><div class="lm-d-h">READOUT</div>'+lmDiag(lm,t,lens).map(x=>'<p>'+x+'</p>').join('')+'</div>';h+=lmCoachBlock(M,key)+lmRecList(M,key,lens);return h;}
...
/* ── tap-to-fullscreen chart: a bigger, interactive version with its own lens
   switcher (change the timeframe in place). Self-contained — own Chart + colors,
   reads the model stashed by the last detail render (LM_FS_CTX). ── */
let LM_FS_CTX=null,LM_FS_CHART=null;
function lmFsCss(v){try{return (getComputedStyle(document.documentElement).getPropertyValue(v)||'').trim()||'#3FD0FF';}catch(e){return '#3FD0FF';}}
function lmFsMk(id,conf){try{if(LM_FS_CHART)LM_FS_CHART.destroy();}catch(e){}var el=document.getElementById(id);if(el&&window.Chart)LM_FS_CHART=new window.Chart(el,conf);}
function lmFsClose(){try{if(LM_FS_CHART){LM_FS_CHART.destroy();LM_FS_CHART=null;}}catch(e){}var h=document.getElementById('lm-fs');if(h)h.innerHTML='';try{document.documentElement.classList.remove('lm-fs-on');}catch(e){}}
function lmFsDraw(lens){if(!LM_FS_CTX)return;const M=LM_FS_CTX.M,metric=LM_FS_CTX.metric,t=lmTileDef(metric),host=document.getElementById('lm-fs');if(!host)return;LM_FS_CTX.lens=lens;
  host.innerHTML='<div class="lm-fs-top"><div class="lm-fs-ttl">'+t.label+' · '+LM_LBL[lens]+'</div><button class="lm-fs-x" onclick="lmFsClose()">✕</button></div>'
    +'<div class="lm-seg lm-fs-seg">'+LM_LENSES.map(l=>'<button class="'+(l[0]===lens?'on':'')+'" onclick="lmFsDraw(\''+l[0]+'\')">'+l[1]+'</button>').join('')+'</div>'
    +'<div class="lm-fs-wrap"><canvas id="lmfschart"></canvas></div>'
    +'<div class="lm-fs-hint">'+(RATIO2[metric]?'two lines = the counts behind the %':'solid = you · dashed = trailing avg · dotted = target')+' — switch the lens above to change the timeframe</div>';
  lmChartMount('lmfschart',M,lens,metric,{mk:lmFsMk,css:lmFsCss});
}
...
function lmChartMount(cv,M,lens,metric,host){var mk=host.mk,css=host.css;try{
  ...
  // 18% headroom so the target/avg lines sit inside the frame, never jammed at the top
  const opt=(unit)=>({plugins:{legend:{display:true,position:'bottom',labels:{boxWidth:big?16:10,font:{size:fs},padding:big?16:8,usePointStyle:true}}},
    scales:{x:{grid:{display:false},ticks:{font:{size:fs},maxRotation:0,autoSkip:true}},
      y:{beginAtZero:true,grace:'18%',title:{display:true,text:unit,font:{size:fs}},ticks:{font:{size:fs}},grid:{color:'rgba(122,176,230,.08)'}}},
    layout:{padding:{top:big?10:2}},interaction:{intersect:false,mode:'index'},maintainAspectRatio:false});
  if(RATIO2[metric]){
    const cc=RATIO2[metric],nd=M.lenses[lens][cc[0]].trend.map(x=>x.value),dd=M.lenses[lens][cc[1]].trend.map(x=>x.value);
    mk(cv,{type:'line',data:{labels:labels,datasets:[
      {label:cc[2][1],data:dd,borderColor:amb,backgroundColor:'transparent',tension:.3,pointRadius:pr,borderWidth:big?2.2:1.6,fill:false,spanGaps:true},
      {label:cc[2][0],data:nd,borderColor:gl,backgroundColor:'rgba(63,208,255,.12)',tension:.3,pointRadius:pr,borderWidth:big?2.8:2,fill:true,spanGaps:true}
    ]},options:opt('count')});
    return;
  }
